...
const DataTable = ({
    ...
    React.useEffect(() => {
        if (dataPageCount !== -1) {
            setNext(true);
            axios.get(`http://localhost8080/H2HBABBA3061/SendData?page=${dataPageCount}`)
                .then((response) => {
    ...
    return ( <
        ...
        <
        InfiniteScroll scrollableTarget = "data-table"
        dataLength = {
            viewSearchResults ? searchResultsLength : dataLength
        }
        hasMore = {
            isNext
        }
        ...
        checked = {
            dataLength > 0 && selectedLength === dataLength
        }
        onChange = {
        ...
        align = "right" > Sales Order Amount < /TableCell> <
        TableCell key = {
            'due_in_date'
        }
        className = {
        ...
        align = "right" > Due Date < /TableCell> <
        TableCell key = {
            'clear_date'
        }
        className = {
        ...
        align = "right" > Predicted Payment Date < /TableCell> <
        TableCell key = {
            'predicted_aging_bucket'
        }
        className = {
            classes.tableHeading
        } > Predicted Aging Bucket < /TableCell> <
        TableCell key = {
            'notes'
        }
        className = {
        ...
        scope = "row" > {
            (searchKeyword === '' ? data : searchResults).map((row) => {
                ...
                return ( <
                    ...
                    onClick = {
                        (event) => handleClick(event, row['doc_id'])
                    }
                    className = {
                        classes.tableRow
                    }
                    ...
                    align = "right" > {
                        row['clear_date'] === '' || row['clear_date'] === null ? '--' : row['clear_date']
                    } < /TableCell> <
                    TableCell className = {
                        classes.tableRow
                    } > -- < /TableCell> <
...
const Bar = ({
    ...
    React.useEffect(() => {
        axios.get(`http://localhost8080/H2HBABBA3061/SearchSalesOrder?searchKeyword=${searchKeyword}&page=${searchPageCount}`)
            .then((response) => {
    ...
    return ( <
        ...
        <
        Button className = {
            isDisabledPredictButton ? classes.DisabledButton : classes.Button
        }
        disabled = {
            isDisabledPredictButton
        } >
        Predict <
        /Button> < /
        div > <
        ...
        <
        Button className = {
            isDisabledViewCorrespondenceButton ? classes.DisabledButton : classes.Button
        }
        onClick = {
            handleViewCorrespondence
        }
        disabled = {
            isDisabledViewCorrespondenceButton
        } >
        View Correspondence <
        /Button> <
        ViewCorrespondencePage open = {
            openViewCorrespondence
        }
        setOpen = {
            setOpenViewCorrespondence
        }
        ...
        <
        Button className = {
            isDisabledEditButton ? classes.DisabledButton : classes.Button
        }
        onClick = {
            handleEditInvoice
        }
        ...
        <
        Button className = {
            isDisabledDeleteButton ? classes.DisabledButton : classes.Button
        }
        onClick = {
            handleDeleteInvoice
        }
        ...
        /Button> <
        DeleteInvoicePage open = {
            openDeleteInvoice
        }
        setOpen = {
            setOpenDeleteInvoice
        }
        selected = {
            selected
        }
