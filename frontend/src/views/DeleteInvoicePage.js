...
const DeleteInvoicePage = ({ open, setOpen, selected, setDataPageCount }) => {
    ...
    const handleDelete = () => {
        axios.post('http://localhost8080/H2HBABBA3061/DeleteSalesOrder', { selected })
            .then((response) => {
    ...
    return ( <
        ...
        div >
        <
        p > You 'll loose your record(s) after this action. We can'
        t recover them once deleted. < /p> <
        p style = {
            { display: 'flex' }
        } >
        <
        ...
        div style = {
            { color: '#FF5E5E', paddingLeft: '0.3rem', paddingRight: '0.3rem' }
        ...
        <
        div style = {
            { paddingRight: '10px', paddingBottom: '10px' }
        } >
        <
